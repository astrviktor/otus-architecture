...
import (
	...
	"otus-architecture/hw12/command/moveto"
...
func TestRunOk(t *testing.T) {
	...
	moveToAdapter := moveto.NewMoveToAdapter(obj)
	moveToCommand := moveto.NewMoveToCommand(moveToAdapter)

	err := moveToCommand.Execute()
	require.Equal(t, nil, err)

	runAdapter := NewRunAdapter(obj)
	runCommand := NewRunCommand(runAdapter)
